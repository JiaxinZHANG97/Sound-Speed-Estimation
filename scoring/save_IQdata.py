...
def save_IQdata(beamformer, moniker, center_angle=False):
    nrois = 12
    ...
    for idx in range(0,1):
        ...
        # Beamform the ROI
        bimg, idelayed_pw, qdelayed_pw = beamformer(P, img_grid)

        outdir = os.path.join("IQdata", moniker, "image")
        if not os.path.exists(outdir):
            os.makedirs(outdir)

        hdf5storage.savemat(
            os.path.join(outdir, "%s%03d" % (data_source, acq)),
            {
    ...
    #     os.makedirs(outdir)
        ...
        plt.suptitle("%s: Image %s%03d" % (moniker, data_source, acq))
        plt.show()
