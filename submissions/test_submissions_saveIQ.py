# File:       test_submissions_saveIQ.py
...
import numpy as np
from cubdl.das_torch_saveIQ import DAS_PW
from submissions.load_submission import load_submission
from scoring.save_IQdata import save_IQdata
...
def load_delayedIQ_all(P, grid, fnum=1):
    ...
    bimg = np.abs(iq)
    bimg /= np.amax(bimg)
    return bimg, idelayed_pw, qdelayed_pw
...
def make_bmode_one(P, grid, fnum=1):
    ...
    for ele in range(nelems):
        ...
        idas, qdas, idas_pw, qdas_pw = das(x)
    idas, qdas = idas.detach().cpu().numpy(), qdas.detach().cpu().numpy()
    idas_pw, qdas_pw = idas_pw.detach().cpu().numpy(), qdas_pw.detach().cpu().numpy()
    iq = idas + 1j * qdas
    bimg = np.abs(iq)
    bimg /= np.amax(bimg)
    return bimg
...
if __name__ == "__main__":
    submissions = [
        ...
        [load_delayedIQ_all, "ground_truth", False],
        # [load_delayedIQ_one, "ground_truth", True],
