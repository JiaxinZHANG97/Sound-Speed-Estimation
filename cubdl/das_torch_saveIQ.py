# File:       das_torch_saveIQ.py
# Author:     Dongwoon Hyun (dongwoon.hyun@stanford.edu)
# Created on: 2020-03-09
# Modified on: 2024-03-13 (Jiaxin Zhang jzhan295@jhu.edu)
import torch
...
class DAS_PW(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        idas_pw = torch.zeros(self.out_shape + (nangles,), dtype=self.dtype, device=self.device)
        qdas_pw = torch.zeros(self.out_shape + (nangles,), dtype=self.dtype, device=self.device)
        # idelayed_pw = torch.zeros(self.out_shape + (nelems, nangles,), dtype=self.dtype, device=self.device)
        # qdelayed_pw = torch.zeros(self.out_shape + (nelems, nangles,), dtype=self.dtype, device=self.device)
        idelayed_pw = torch.zeros((npixels, nelems, nangles), dtype=self.dtype, device=self.device)
        qdelayed_pw = torch.zeros((npixels, nelems, nangles), dtype=self.dtype, device=self.device)
        ...
        for t, td, ta in zip(self.ang_list, txdel, txapo):
            ...
            for r, rd, ra in zip(self.ele_list, rxdel, rxapo):
                ...
                idas += ifoc * apods
                qdas += qfoc * apods
            idas_pw[:, :, t] = idas.view(self.out_shape)
            qdas_pw[:, :, t] = qdas.view(self.out_shape)
        ...
        # idelayed_pw = idelayed_pw.sum(axis=3, keepdim=False)
        # qdelayed_pw = qdelayed_pw.sum(axis=3, keepdim=False)
        return idas, qdas, idelayed_pw, qdelayed_pw
